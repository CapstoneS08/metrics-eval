...
def _call_openai_json(text: str, system_prompt: str, model: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"improvement_comment": raw}


# --- Anthropic / Claude ---
def _get_anthropic_client():
    # anthropic package must be installed where you run this
...
def _call_claude_json(text: str, system_prompt: str, model: str) -> Dict[str, Any]:
    ...
    msg = client.messages.create(
        model=model,  # e.g. "claude-sonnet-3-5"
        max_tokens=200,
        temperature=0.0,
...
def call_llm_json(text: str, system_prompt: str, model: str) -> Dict[str, Any]:
    """
    Routes by model string:
      - if model starts with "claude-" => Anthropic
      - else => OpenAI
    """
    if model.startswith("claude-"):
        return _call_claude_json(text, system_prompt, model)
    return _call_openai_json(text, system_prompt, model)
